...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/health") {
      return new Response(JSON.stringify({ status: "ok", app: "emia-edutech", timestamp: (/* @__PURE__ */ new Date()).toISOString() }), {
    ...
    if ((url.pathname === "/api/hotmart-webhook" || url.pathname === "/api/webhook") && request.method === "POST") {
      try {
        ...
        const buyerEmail = (data?.data?.buyer?.email || // Hotmart 2.0
        ...
        "").trim().toLowerCase();
        const event = (data?.event || data?.status || data?.order_status || data?.trans_status || "").toLowerCase();
        const isApproved = event.includes("approved") || event.includes("paid") || event.includes("completed") || event.includes("pago") || event.includes("autorizado") || event.includes("purchase_approved") || event === "3";
        const productName = (data?.data?.product?.name || data?.Product?.name || data?.product_name || data?.prod_name || "").toLowerCase();
      ...
      try {
        ...
        if (contentType.includes("multipart/form-data")) {
          ...
          if (!prompt && form.get("title")) {
            prompt = `Elabore um trabalho acad\xEAmico sobre ${form.get("title")}`;
          }
        ...
        if (apiKeysToTry.length === 0) {
          return new Response(JSON.stringify({
            success: false,
            error: "Chave Gemini n\xE3o configurada no servidor. Configure a vari\xE1vel GEMINI_API_KEY ou insira sua chave no app."
          }), {
            status: 400,
            headers: { "Content-Type": "application/json", ...corsHeaders }
          });
            ...
            while (attempts < maxAttempts) {
              try {
                ...
                if (geminiRes.status === 429 || geminiRes.status === 500 || geminiRes.status === 503) {
                  attempts++;
                  const delay = Math.pow(2, attempts) * 1e3 + Math.random() * 500;
    ...
    if (url.pathname === "/api/tts" && request.method === "POST") {
      try {
        ...
        if (ttsRes.ok) {
          ...
          return new Response(JSON.stringify({ error: "Falha ao sintetizar \xE1udio neural" }), {
            status: 502,
            headers: { "Content-Type": "application/json", ...corsHeaders }
          });
        }
